...
//
// SearchService — calls Tavily API for web search
...
impl SearchService {
    ...
    pub async fn search(&self, query: &str) -> Result<SearchResponse, anyhow::Error> {
        ...
        Ok(SearchResponse {
            results: tavily.results.into_iter().map(|r| SearchResult {
                title: r.title,
                url: r.url,
                content: r.content,
            }).collect(),
