...
use qdrant_client::qdrant::{
    ...
    Condition, CreateCollection, Distance, Filter, GetPoints, PointId, PointStruct, 
    ScrollPoints, SearchPoints, Vector, Vectors, WithPayloadSelector, 
    WithVectorsSelector, WriteOrdering, VectorParams, VectorsConfig,
...
impl VectorDbService {
    ...
    pub async fn ensure_collection(&self, name: &str, dim: u64) -> Result<()> {
        if !self.client.collection_exists(name).await? {
            ...
            for field in index_fields {
                let _ = self.client.create_field_index(name, field, qdrant_client::qdrant::FieldType::Keyword, None, None).await;
            }
        }
        Ok(())
    }
    ...
    pub async fn upsert_points(
        &self,
        collection_name: &str,
        points_data: Vec<(Uuid, Vec<f32>, std::collections::HashMap<String, qdrant_client::qdrant::Value>)>,
    ) -> Result<()> {
        ...
        let ordering = Some(WriteOrdering { r#type: WriteOrderingType::Strong.into() });

        self.client.upsert_points(collection_name, None, points, ordering).await?;
        tracing::info!("Qdrant: Upserted {} points into {} with Strong ordering.", count, collection_name);

        if let Some(fid) = first_id {
            let pid = PointId { point_id_options: Some(PointIdOptions::Uuid(fid.to_string())) };
            let res = self.client.get_points(
                collection_name,
                None,
                &[pid],
    ...
    pub async fn delete_points(&self, collection_name: &str, ids: Vec<Uuid>) -> Result<()> {
        let pids: Vec<PointId> = ids.into_iter().map(|id| PointId {
            point_id_options: Some(PointIdOptions::Uuid(id.to_string()))
        }).collect();
        ...
        self.client.delete_points(collection_name, None, &selector, None).await?;
        Ok(())
    }
    ...
    pub async fn scroll(
        ...
        let filter = self.build_filter(filters);
        let res = self.client.scroll(&ScrollPoints {
            collection_name: collection_name.to_string(),
            filter,
            limit,
            with_payload: Some(true.into()),
