...
impl AgentRepository {
    ...
    pub async fn update(
        ...
        // workflow_id: only apply if explicitly provided in the request
        // Some(Some(id)) → set association, Some(None) → clear association, None → don't touch
        if let Some(workflow_id) = req.workflow_id {
            active.workflow_id = Set(workflow_id);
        }
