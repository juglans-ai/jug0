...
pub async fn tool_result_handler(
    ...
    if let Ok(chat_id) = Uuid::parse_str(&req.call_id) {
        ...
        if let Some(sender) = state.tool_result_channels.get(&chat_id) {
            ...
            sender.send(req.results.clone()).await.map_err(|_| {
...
pub async fn chat_handler(
    ...
    if let Some(ref agent) = agent_opt {
        if let Some(workflow_id) = agent.workflow_id {
            ...
            tracing::info!(
                "🔐 [Forward] Forwarding chat to workflow '{}' at {} with execution token (caller: {}, author: {}, chat_id: {})",
                workflow.slug, chat_url, user.id, agent.user_id.unwrap_or(user.id), chat_id
            );
    ...
    let tool_result_rx = if req_stream {
        let (tx, rx) = tokio::sync::mpsc::channel::<Vec<types::ToolResultPayload>>(1);
