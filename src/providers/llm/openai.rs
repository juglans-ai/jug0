...
use super::{LlmProvider, ChatStreamChunk, ToolCallChunk};
...
impl OpenAIProvider {
    pub fn new() -> Self {
        ...
        let raw_base = std::env::var("OPENAI_API_BASE").unwrap_or_else(|_| "https://api.openai.com/v1".to_string());
    ...
    fn build_user_content(&self, parts_json: &serde_json::Value) -> ChatCompletionRequestUserMessageContent {
        ...
        if let Ok(parts) = serde_json::from_value::<Vec<MessagePart>>(parts_json.clone()) {
            for part in parts {
                if let Some(text) = part.content { content_parts.push(ChatCompletionRequestMessageContentPart::Text(ChatCompletionRequestMessageContentPartTextArgs::default().text(text).build().unwrap())); }
                // Image 等逻辑保持不变
            }
        }
        if content_parts.is_empty() { ChatCompletionRequestUserMessageContent::Text("".to_string()) } else { ChatCompletionRequestUserMessageContent::Array(content_parts) }
    }
}
        ...
        let mapped_stream = stream.map(|item| {
            match item {
                Ok(resp) => {
                    ...
                    Ok(ChatStreamChunk { content, tool_calls: tool_chunks, usage: None, finish_reason: None })
                },
