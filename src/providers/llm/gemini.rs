...
impl GeminiProvider {
    ...
    fn find_function_name_by_id(history: &[messages::Model], target_id: &str) -> Option<String> {
        for msg in history.iter().rev() {
            if msg.role == "assistant" {
                if let Some(tools_json) = &msg.tool_calls {
                    if let Ok(calls) = serde_json::from_value::<Vec<serde_json::Value>>(tools_json.clone()) {
                        for call in calls {
                            if let Some(id) = call.get("id").and_then(|v| v.as_str()) {
                                if id == target_id {
                                    return call.get("function").and_then(|f| f.get("name")).and_then(|n| n.as_str()).map(|s| s.to_string());
                                }
                            }
                        }
                    }
    ...
    fn build_gemini_parts(&self, parts_json: &serde_json::Value) -> Vec<Part> {
                        ...
                        if let Some(data_val) = part.data {
                            if let Some(data_str) = data_val.as_str() {
                                if data_str.starts_with("data:") {
                                    ...
                                    if parts.len() == 2 {
                                        let meta = parts[0];
                                        let mime = meta.split(';').next().unwrap_or("").replace("data:", "");
                                        let b64 = parts[1];
                                        gemini_parts.push(Part::InlineData {
                                            inline_data: InlineData { mime_type: mime, data: b64.to_string() }
                                        });
                                    }
                                } else {
                                    gemini_parts.push(Part::Text { text: format!("[Image URL: {}]", data_str) });
                                }
                            }
                        }
                    },
        ...
        if gemini_parts.is_empty() {
            gemini_parts.push(Part::Text { text: "".to_string() });
        }
        gemini_parts
    }
}
...
impl LlmProvider for GeminiProvider {
    async fn stream_chat(
        ...
        for (i, msg) in history.iter().enumerate() {
            match msg.role.as_str() {
                ...
                "user" => {
                    ...
                    gemini_contents.push(Content { role: "user".to_string(), parts });
                },
                "assistant" => {
                    let mut parts = Vec::new();
                    let text_content = if let Ok(p) = serde_json::from_value::<Vec<MessagePart>>(msg.parts.clone()) {
                         p.first().and_then(|pp| pp.content.clone()).unwrap_or_default()
                    } else { String::new() };
                    ...
                    if let Some(tc_json) = &msg.tool_calls {
                        ...
                        if is_next_tool {
                            if let Ok(calls) = serde_json::from_value::<Vec<serde_json::Value>>(tc_json.clone()) {
                                for call in calls {
                                    if let Some(func) = call.get("function") {
                                        ...
                                        let sig = call.get("id").and_then(|s| s.as_str()).map(|s| s.to_string());
                    ...
                    if !parts.is_empty() {
                        gemini_contents.push(Content { role: "model".to_string(), parts });
                    }
                },
                "tool" => {
                    let tool_id = msg.tool_call_id.clone().unwrap_or_default();
                    let func_name = Self::find_function_name_by_id(&history, &tool_id).unwrap_or_else(|| "unknown_tool".to_string());
                    
                    let content_str = if let Ok(p) = serde_json::from_value::<Vec<MessagePart>>(msg.parts.clone()) {
                         p.first().and_then(|pp| pp.content.clone()).unwrap_or_default()
                    } else { String::new() };
                    
                    let response_json: serde_json::Value = serde_json::from_str(&content_str).unwrap_or(json!({ "result": content_str }));

                    gemini_contents.push(Content {
                        role: "function".to_string(),
                        parts: vec![Part::FunctionResponse {
                            function_response: FunctionResponseData { name: func_name, response: response_json }
                        }]
                    });
                }
                _ => {}
        ...
        let gemini_tools = tools.map(|ts| {
            ...
            vec![ToolWrapper { function_declarations: decls }]
        });
        ...
        let url = format!("{}/{}:streamGenerateContent?alt=sse&key={}", self.base_url, model, self.api_key);
        ...
        let mapped_stream = stream.map(|item| {
            match item {
                Ok(event) => {
                    if event.data == "[DONE]" {
                        return Ok(ChatStreamChunk { content: None, tool_calls: vec![], usage: None, finish_reason: None });
                    }

                    let resp: GeminiResponse = match serde_json::from_str(&event.data) {
                        Ok(r) => r,
                        Err(_) => return Ok(ChatStreamChunk { content: None, tool_calls: vec![], usage: None, finish_reason: None })
                    };
                        ...
                        if let Some(first) = candidates.first() {
                            if let Some(content) = &first.content {
                                if let Some(parts) = &content.parts {
                                    for part in parts {
                                        match part {
                                            ResponsePart::Text { text } => { content_acc = Some(text.clone()); },
                                            ResponsePart::FunctionCallWrapper { function_call, thought_signature } => {
                                                let gemini_id = thought_signature.clone();
                                                let synthetic_id = format!("call_{}", Uuid::new_v4().simple());
                                                let name = function_call.get("name").and_then(|s| s.as_str()).map(|s| s.to_string());
                                                let args = function_call.get("args").map(|v| v.to_string());
                    ...
                    Ok(ChatStreamChunk { content: content_acc, tool_calls: tool_calls_acc, usage, finish_reason: None })
                },
